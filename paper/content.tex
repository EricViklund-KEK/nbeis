...
Electropolishing (EP) is a common method to polish Nb superconducting radiofrequency (SRF) cavities. EP uses an HF-containing electrolyte to dissolve the surface of the Nb when a voltage is applied. In ideal conditions, EP can acheive nanometer-scale surface roughness. However, defects such as etching and pitting can sometimes occur, especially when polishing large cavities. One of the main causes of these defects has been shown to be insufficient voltage applied to the cavity during EP.\cite{chouhan2022study,viklund2022studies,chouhan2023electropolishing} To better understand why this is the case, we utilize a technique known as electrochemical impedance spectroscopy (EIS) to investigate the surface chemistry of Nb in the HF electrolyte.

There are two theorized models to explain the mechanism of niobium EP, the viscous layer model and the compact film model. The viscous layer model supposes that polishing is caused by the formation of boundary layer near the niobium surface consisting of dissolved Nb reaction byproducts. The reaction is limited by the diffusion of HF through this viscous layer. Polishing occurs because of the reduced layer thickness around surface protrusions leading to increased HF diffusion and a higher local removal rate. The compact film model in contrast is based on the formation of a solid passivating film on the surface of the Nb which homogenizes the surface and prevents etching due to grain orientation effects. In this study we find evidence supporting the compact film model using electrochemical impedance spectroscopy (EIS) to analyze the chemical properties of the Nb at different polishing voltages.

EIS has been used to study the chemistry of niobium EP previously.\cite{cattarin2002nb, Tian_2008, tian2008novel, ranjith2018anodic} However, these studies do not fully explain the observed impedance characteristics of Nb EP, especially at the low frequency end of the impedance spectrum. To accomplish this, we use a method of analyzing the EIS spectrum called distribution of relaxation times (DRT) analysis.\cite{10.1063/1.1745355, wan2015influence, ZHANG2015464} DRT analysis transforms the impedance spectrum in to a relaxation time distribution function. This function consists of peaks corresponding to the relaxation time of chemical processes on the electrode surface. This method has been used to characterize a wide range of electrochemical systems such as fuel cells,\cite{Sonn_2008, schichlein2002deconvolution, Leonide_2008} batteries,\cite{SCHMIDT201370, batteries5020043, SONI202297} and supercapacitors.\cite{HELSETH2019100912} Since the DRT method does not require a model of the electrode chemical reaction, like traditional equivalent circuit fitting methods, it is well suited for complex systems that are not well understood such as Nb EP. 

In most of the electrochemistry literature, only the positive positive relaxation times are included in the DRT. However, this is insufficient to explain the Nb EP system in the low frequency region. To model this region, we generalize the DRT method to negative relaxation times. In section \ref{sec:org7d749e2} we show that the relaxation times of the system correspond to the eigenvalues of the open loop transfer function of the system which can take on both positive and negative values. Using this generalized DRT method, we are able to fit the experimental data very well at all polishing voltages. We observe two DRT peaks which appear at higher polishing voltages which we theorize to be caused by the formation of an NbO and an Nb\textsubscript{2}O\textsubscript{5} layer on the Nb electrode. The formation of the Nb\textsubscript{2}O\textsubscript{5} oxide also corresponds with a reduction in surface etching indicating this oxide layer is necessary for good polishing.
...
We have created a custom python script to analyze the EIS measurements. This script consists of two parts, numerical integration, and normalized linear least squares fitting. The first part is the numerical integration of equation \ref{eq:gDRT} for a particular set of basis functions. We use the set of log-gaussian functions as our basis set. The second part is the normalized least squares fitting algorithm, which is available in the SciPy python package \cite{2020SciPy-NMeth}. The normalization method used is based on minimizing the derivatives of the DRT to eliminate oscillations caused by over-fitting. More information on the algorithms used can be found in section \ref{sec:sup}.
...
Using the generalized DRT method, we are able to fit the Nb impedance spectrum across all frequencies including the inductive mid-frequency loop and the low frequency loop as seen in figure \ref{fig:bodeplot}. From figure \ref{fig:gamma} we can see that there are three prominent peaks in the DRT. The peak centered around \qtyrange{100}{1000}{\hertz} corresponds to the electric double layer capacitance. The center of this peak shifts to the right linearly with the polishing voltage. Since the frequency is plotted on a logarithmic scale, this shows that time constant of the electric double layer follows an Arrhenius type relation. This is consistent with the Gouy-Chapman theory of the electrical double layer.

The two other peaks centered around \qty{10}{\hertz}, one with a positive eigenvalue and one with a negative eigenvalue. These peaks likely correspond to niobium oxidation reactions. The negative peak with a maximum around \qtyrange{0.7}{0.75}{\volt} is caused by the oxidation of Nb to NbO, while the positive peak with a maximum at \qtyrange{0.8}{0.9}{\volt} is caused by the oxidation of NbO to Nb\textsubscript{2}O\textsubscript{5}. This can be verified by XPS measurements of Nb exposed to HF electrolytes in the active and passive regimes \cite{ranjith2018anodic}. The NbO peak first appears in the active region of the current voltage curve and reaches a maximum when the Nb transitions to the passive region. The Nb\textsubscript{2}O\textsubscript{5} peak appears in the active to passive transition region and grows larger in the passive region. This indicates that the Nb surface is becoming more oxidized as the polishing voltage increases and more of the surface is being covered by Nb\textsubscript{2}O\textsubscript{5}. 
